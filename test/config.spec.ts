...
test('config.ts defaults', (assert) => {
  ...
  assert.deepEqual(spy.args[0], [moduleTest.get('TestModule')[0], testconfig], 'should call lodash.defaultsDeep with arguments');
  sandbox.restore();
  assert.end();
});
...
test('config.ts defaults', (assert) => {
  ...
  let moduleTest = new ConfigModule();
  moduleTest.defaults(testconfig);
  assert.deepEqual(moduleTest.get('TestModule')[0], testconfig, 'push configuration to array if it doesnt exit');
  assert.end();
});
...
test('config.ts merge', (assert) => {
  ...
  assert.deepEqual(spy.args[0], [moduleTest.get('TestModule')[0], testconfig], 'should call lodash.merge with arguments');
  sandbox.restore();
  assert.end();
});
